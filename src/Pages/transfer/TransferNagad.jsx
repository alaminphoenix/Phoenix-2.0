...
const TransferNagad = () => {
  // data of clint from redux
  const ClintData = useSelector((state) => state.info.userdata);

  // manage state
  const [category] = useState("Nagad");
  const [categoryPhoto] = useState(
    "https://downloadr2.apkmirror.com/wp-content/uploads/2020/07/10/5f094774bdb14-384x384.png"
  );
    ...
    <>
      <div className=" transferMain text-white ">
        ...
        <div className="px-2 md:px-[100px]">
          ...
          <button onClick={NagadSetData} className=" w-full bg-white text-blue-700 mt-10 rounded-lg font-bold hover:scale-95 active:scale-100 transition-all py-4 ">
            {" "}
            Send{" "}
          </button>
        </div>
